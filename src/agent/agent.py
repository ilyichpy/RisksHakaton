...
def check_file_info():
    ...
    1) Проверь есть ли  различия в персональных данных от разных банков в файле и если найдется, выведи какие различия ты нашел. ТОЛЬКО выведи разные значения фамилии, имени, отчества, даты рождения и паспорта. Клиент сам должен принять решение - это ошибка или нет. Главное покажи ему данные.
    ...
    2) обязательно Выведи индивидуальный рейтинг клиента и рекомендации к нему.

    3) Дай краткую аналитику по кредитной истории из текста в пяти пунктах.
    ...
    - используй для анализа только данные из текста;
    Не выдумывай данные, не генерируй данные, которые противоречат данным из файла, анализируй только данные из файла.
    Если пользователь ввел новую информацию или новый файл, то почисть историю переписки.
    """
